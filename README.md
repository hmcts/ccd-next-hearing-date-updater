...
> 1. Stop and / or delete the `cftlib` containers generated by [bootWithCcd](#bootwithccd); to avoid port number
     conflicts.
> 2. Delete the [.env.test.stub.service.env](./.env.test.stub.service.env) file; as the test-stub URL override is no
     longer required.
> 3. (if testing manually) After starting the local [CCD-Docker](https://github.com/hmcts/ccd-docker)
     instance, re-import the [Test Case Types](#test-case-types) definitions; as the test-stub/callback URLs used by
